...
serve(async (req) => {
  ...
  try {
    const { messages, executeRegistration, registrationData } = await req.json();
    ...
    // Derive userId from the verified JWT (never trust body)
    ...
    // Registration mutations require an authenticated user
    if (executeRegistration && !userId) {
