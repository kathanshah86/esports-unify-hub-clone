...
const RegistrationFormDialog: React.FC<RegistrationFormDialogProps> = ({
  ...
  // Battle Code (referral) state
  const [referralInput, setReferralInput] = useState('');
  const [appliedReferralCode, setAppliedReferralCode] = useState<string | null>(null);
  const [referrerUserId, setReferrerUserId] = useState<string | null>(null);
  const [validatingReferral, setValidatingReferral] = useState(false);

  const discountAmount = appliedReferralCode && referralEnabled
    ? Math.floor((entryFee * (referralDiscountPercent || 0)) / 100)
    : 0;
  const effectiveFee = Math.max(0, entryFee - discountAmount);
  const hasEnoughBalance = walletBalance >= effectiveFee;
  ...
  useEffect(() => {
    if (open) {
      ...
      // Reset referral state when dialog closes
      setReferralInput('');
  ...
  // Re-evaluate wallet pay option when discount changes
  useEffect(() => {
    if (isPaid) setPayViaWallet(walletBalance >= effectiveFee);
  }, [effectiveFee, walletBalance, isPaid]);
  ...
  const handleApplyReferral = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('validate_referral_code' as any, { _code: code });
      if (error) throw error;
      const referrer = data as string | null;
      if (!referrer) {
        toast({ title: 'Invalid Code', description: 'This Battle Code does not exist.', variant: 'destructive' });
        return;
      }
      if (user && referrer === user.id) {
        toast({ title: 'Not Allowed', description: "You can't use your own Battle Code.", variant: 'destructive' });
        return;
      }
      setAppliedReferralCode(code);
      setReferrerUserId(referrer);
      toast({ title: 'Code Applied!', description: `${referralDiscountPercent}% discount applied to your entry fee.` });
    } catch (e: any) {
      toast({ title: 'Error', description: e.message || 'Failed to validate code', variant: 'destructive' });
    } finally {
  ...
  const loadWalletBalance = async () => {
    ...
    try {
      ...
      setPayViaWallet(bal >= effectiveFee);
    } catch (e) {
  ...
  const uploadScreenshot = async (): Promise<string | undefined> => {
    ...
    try {
      ...
      // Store the storage path (bucket is private); admin will resolve via signed URL
      return filePath;
    } catch (error) {
            ...
            <div className="mt-4 flex items-center gap-2 flex-wrap">
              <Badge className={`${isPaid ? 'bg-amber-500/20 text-amber-300 border-amber-500/30' : 'bg-green-500/20 text-green-300 border-green-500/30'} px-3 py-1 text-sm`}>
                ...
                {isPaid ? (
                  appliedReferralCode ? (
                    <span>
                      <span className="line-through opacity-60 mr-1">₹{entryFee}</span>
                      ₹{effectiveFee}
                    </span>
                  ) : (
                    `Entry Fee: ₹${entryFee}`
              ...
              {appliedReferralCode && (
                <Badge className="bg-green-500/20 text-green-300 border-green-500/30 px-3 py-1 text-sm">
                  -{referralDiscountPercent}% applied
                </Badge>
              )}
              {totalFields > 0 && (
            ...
            <div className="space-y-5 px-6 py-4">
              ...
              {isPaid && (
                <div className="space-y-4">
                  ...
                  {/* Battle Code (Referral) Input */}
                  {referralEnabled && referralDiscountPercent > 0 && (
                    <div className="p-3 bg-purple-500/5 border border-purple-500/30 rounded-xl space-y-2">
                      <Label className="text-purple-200 font-medium text-sm flex items-center gap-2">
                        <Trophy className="w-3.5 h-3.5" />
                        Have a Battle Code? Get {referralDiscountPercent}% off
                      </Label>
                      {appliedReferralCode ? (
                        <div className="flex items-center gap-2">
                          <div className="flex-1 bg-green-500/10 border border-green-500/30 rounded-lg px-3 py-2 flex items-center justify-between">
                            <code className="text-green-300 font-mono font-bold tracking-widest">
                              {appliedReferralCode}
                            </code>
                          ...
                          <Button
                            ...
                            className="bg-transparent border-red-500/40 text-red-300 hover:bg-red-500/10 h-9"
                          >
                            Remove
                          </Button>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <Input
                            ...
                            className="bg-gray-800/60 border-purple-500/30 text-white placeholder-gray-500 font-mono tracking-widest h-9"
                            maxLength={20}
                          />
                          <Button
                            type="button"
                  ...
                  {loadingWallet ? (
                    ...
                    <div className="p-4 bg-green-500/10 border border-green-500/30 rounded-xl space-y-3">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-amber-300 font-bold">- ₹{effectiveFee}{appliedReferralCode && <span className="text-xs ml-1 text-green-300">(saved ₹{discountAmount})</span>}</span>
                      </div>
                      <div className="h-px bg-gray-700" />
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-gray-300">Remaining:</span>
                        <span className="text-white font-bold">₹{walletBalance - effectiveFee}</span>
                      </div>
                      <p className="text-xs text-green-200/70">
                        ₹{effectiveFee} will be deducted from your wallet automatically.
                      </p>
                    </div>
                  ) : (
                    ...
                    <>
                      ...
                      <div className="p-4 bg-blue-500/5 border border-blue-500/20 rounded-xl space-y-2">
                        ...
                        <ol className="text-xs text-blue-200/80 space-y-2">
                          {['Scan the QR code using any UPI app', `Pay exactly ₹${effectiveFee}`, 'Take a screenshot of success', 'Upload it below'].map((step, i) => (
