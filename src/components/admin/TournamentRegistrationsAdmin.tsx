    ...
    const loadScreenshotUrls = async () => {
      ...
      for (const reg of registrations) {
        if (reg.payment_screenshot_url) {
          try {
            const raw = reg.payment_screenshot_url;
            // Extract storage path: handle legacy full URLs that embed the bucket path
            let path = raw;
            ...
            if (idx !== -1) {
              ...
              // Different bucket / external URL — use as-is
              urlMap[reg.id] = raw;
              continue;
            }
    ...
    try {
      ...
      if (referrerIds.length > 0) {
        const { data: profs } = await supabase
          .from('profiles')
          .select('user_id, username, display_name, email, referral_code')
          .in('user_id', referrerIds);
        const map = new Map((profs || []).map(p => [p.user_id, p]));
        regs.forEach(r => {
          if (r.referrer_user_id) {
            const p = map.get(r.referrer_user_id);
            if (p) {
              r.referrer_name = p.display_name || p.username || p.email || null;
              r.referrer_email = p.email || null;
            }
          }
        });
      }
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          {registration.referral_code_used && (
                            <div className="mt-2 inline-flex flex-col gap-1 bg-purple-900/30 border border-purple-500/40 rounded-lg px-3 py-2 text-xs">
                              <div className="text-purple-300 font-semibold">
                                Battle Code Used: <span className="font-mono text-white">{registration.referral_code_used}</span>
                              </div>
                              <div className="text-gray-300">
                                Used by: <span className="text-white">{registration.player_name}</span>
                              </div>
                              <div className="text-gray-300">
                                Code owner:{' '}
                                <span className="text-emerald-300">
                                  {registration.referrer_name || registration.referrer_email || (registration.referrer_user_id ? registration.referrer_user_id.slice(0, 8) + '…' : 'Unknown')}
                                </span>
                              </div>
                            </div>
                          )}
