...
const Wallet = forwardRef<HTMLDivElement>((_, ref) => {
  ...
  const handleRedeemBattleCode = async () => {
    ...
    if (!battleCode.trim()) {
      toast({
        title: "Code Required",
        description: "Please enter a battle coin",
        variant: "destructive"
      });
      return;
    }
  ...
  const CREDIT_TYPES = ['deposit', 'battle_code', 'prize_winning'];
  ...
  const getTransactionTitle = (type: string) => {
    switch (type) {
      ...
      case 'battle_code': return 'Refer & Earn Reward';
      case 'prize_winning': return 'Prize Winning';
                ...
                >
                  ...
                  <div className="flex items-center justify-between sm:justify-end space-x-3 sm:space-x-4 pl-14 sm:pl-0">
                    <div className={`font-bold text-base sm:text-lg ${
                      ...
                      {isCredit(transaction.transaction_type) ? '+' : '-'}₹{transaction.amount.toFixed(0)}
                    </div>
        ...
        <DialogContent className="bg-gray-900 border border-gray-700 text-white max-w-md">
          ...
          <div className="space-y-6 mt-4">
            <p className="text-gray-400 text-sm">
              Enter your battle coin to add bonus money to your wallet instantly!
            </p>
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">
                Battle Coin
        ...
        <DialogContent className="bg-gray-900 border border-gray-700 text-white max-w-md max-h-[90vh] overflow-y-auto">
          <DialogHeader>
              ...
              <div className={`inline-flex items-center justify-center w-10 h-10 rounded-xl mr-3 ${
                selectedTransaction && isCredit(selectedTransaction.transaction_type) ? 'bg-green-500' : 'bg-orange-500'
              }`}>
                {selectedTransaction && isCredit(selectedTransaction.transaction_type)
                  ? <Plus className="w-5 h-5 text-white" />
                  : <Minus className="w-5 h-5 text-white" />
                }
              </div>
            ...
            <div className="space-y-4 mt-4">
              ...
              <div className="p-4 bg-gray-800/50 rounded-xl border border-gray-700/30">
                ...
                <p className={`text-2xl font-bold ${
                  isCredit(selectedTransaction.transaction_type) ? 'text-green-400' : 'text-red-400'
                }`}>
                  {isCredit(selectedTransaction.transaction_type) ? '+' : '-'}₹{selectedTransaction.amount.toFixed(0)}
                </p>
              </div>
              ...
              <div className="p-4 bg-gray-800/50 rounded-xl border border-gray-700/30">
                ...
                <p className="text-white font-medium">
                  {getTransactionTitle(selectedTransaction.transaction_type)}
                </p>
              </div>
