...
const Admin = () => {
  ...
  const handleSaveTournament = async () => {
    try {
      ...
      const tournamentData = {
        ...
        referral_enabled: !!tournamentForm.referral_enabled,
        referral_discount_percent: Math.max(0, Math.min(100, parseInt(tournamentForm.referral_discount_percent as any) || 0)),
      } as any;
  ...
  const startEditTournament = (tournament: Tournament) => {
    ...
    setTournamentForm({
      ...
      referral_enabled: !!tournament.referral_enabled,
      referral_discount_percent: tournament.referral_discount_percent ? String(tournament.referral_discount_percent) : '',
    });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <Tabs defaultValue="tournaments" className="space-y-6">
          <TabsList className="grid w-full grid-cols-6 lg:grid-cols-14 bg-gray-800 h-auto flex-wrap">
            ...
            <TabsTrigger value="all-details" className="data-[state=active]:bg-purple-500">
              <TableIcon className="w-4 h-4 mr-2" />
              All Details
            </TabsTrigger>
          </TabsList>
            ...
            {(showAddTournament || editingTournament) && (
              <Card className="bg-gray-800 border-gray-700">
                ...
                <CardContent className="space-y-4">
                  ...
                  {/* Referral / Battle Code Discount */}
                  {tournamentForm.entry_fee_type === 'paid' && (
                    <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <div>
                          <label className="block text-sm font-medium text-amber-300">
                            Enable Battle Code (Referral) Discount
                          </label>
                          <p className="text-xs text-gray-400 mt-1">
                            Let users paste a friend's Battle Code to get a discount on the entry fee.
                          </p>
                        </div>
                        <Switch
                          checked={tournamentForm.referral_enabled}
                          onCheckedChange={(checked) =>
                            setTournamentForm({ ...tournamentForm, referral_enabled: checked })
                          }
                        />
                      </div>
                      {tournamentForm.referral_enabled && (
                        <div>
                          <label className="block text-sm font-medium text-gray-300 mb-2">
                            Discount Percentage (%)
                          </label>
                          <Input
                            ...
                            min="1"
                            max="100"
                            value={tournamentForm.referral_discount_percent}
                            onChange={(e) =>
                              setTournamentForm({ ...tournamentForm, referral_discount_percent: e.target.value })
                            }
                            placeholder="e.g., 10 for 10% off"
                            className="bg-gray-700 border-gray-600 text-white"
                          />
                          <p className="text-xs text-gray-400 mt-1">
                            Discount is applied automatically when a valid Battle Code is entered at registration.
                          </p>
                        </div>
                      )}
                    </div>
